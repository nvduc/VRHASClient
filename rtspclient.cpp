...
bandwidth_estimator_update(unsigned int ssrc, unsigned short seq, struct timeval rcvtv, unsigned int timestamp, unsigned int pktsize) {
	...
	if((seq-1) == mi->second.lastPktSeq) {
		if(sampleframe) {
			...
			// if(mi->second.framecount >= FRAME_RATE && mi->second.samples >= 3000) {
			printf("Frame #%d\n", mi->second.framecount);//
			if(mi->second.framecount % FRAME_RATE == 0){
				ctrlmsg_t m;
...
#ifdef SAVE_ENC // initiate variables here
	if(fout == NULL) {
...
DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
...
#endif
	struct timeval start_play_t;
	struct timeval finish_play_t;
	double client_proc_delay;	
	...
	if(fSubsession.rtpPayloadFormat() == video_sess_fmt) {
		...
		gettimeofday(&start_play_t, NULL);
		play_video(channel,
			fReceiveBuffer+MAX_FRAMING_SIZE-video_framing,
			frameSize+video_framing, presentationTime,
			marker);
		gettimeofday(&finish_play_t, NULL);
		client_proc_delay = tvdiff_us(&finish_play_t, &start_play_t)/1000.0;
		printf("$$$$$$$$$$$$$$$$ client_proc_delay: %.2f\n", client_proc_delay);
#ifdef ANDROID
		if(rtspconf->builtin_video_decoder==0
